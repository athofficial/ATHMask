...
versionedData.meta.version=versionconst version=8
const extend=require('xtend')
const clone=require('clone')
module.exports={version,migrate:function(originalVersionedData){const versionedData=clone(originalVersionedData)
...
function transformState(state){const newState=extend(state,{NoticeController:{noticesList:state.noticesList||[],},})
...
versionedData.data=newState}catch(err){console.warn(`MetaMask Migration #${version}`+err.stack)}
return Promise.resolve(versionedData)},}
function transformState(state){const newState=extend(state,{TransactionManager:{transactions:state.transactions||[],gasMultiplier:state.gasMultiplier||1,},})
