...
QUnit.test('render init screen',(assert)=>{const done=assert.async()
runFirstTimeUsageTest(assert).then(done).catch((err)=>{assert.notOk(err,`Error was thrown: ${err.stack}`)
done()})})
async function runFirstTimeUsageTest(assert,done){if(window.METAMASK_PLATFORM_TYPE==='mascara'){return runMascaraFirstTimeTest(assert,done)}
const app=$('#app-content')
while(!0){const button=app.find('button')
if(button.html()==='Accept'){const termsPage=app.find('.markdown')[0]
termsPage.scrollTop=termsPage.scrollHeight
await timeout()
console.log('Clearing notice')
button.click()
await timeout()}else{console.log('No more notices...')
break}}
await timeout()
...
function timeout(time){return new Promise((resolve,reject)=>{setTimeout(resolve,time||1500)})}