...
async function runFirstTimeUsageTest (assert, done) {
  ...
  await timeout()const PASSWORD='password123'
const reactTriggerChange=require('react-trigger-change')
async function runFirstTimeUsageTest(assert,done){await timeout(4000)
const app=$('#app-content')
await skipNotices(app)
await timeout()
...
const selectPhrase=text=>{const option=$('.backup-phrase__confirm-seed-option').filter((i,d)=>d.textContent===text)[0]
$(option).click()}
await timeout(1000)
...
async function skipNotices(app){while(!0){const button=app.find('button')
if(button&&button.html()==='Accept'){const termsPage=app.find('.markdown')[0]
if(!termsPage){break}
